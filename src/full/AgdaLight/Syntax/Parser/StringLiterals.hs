{-| The code to lex string and character literals. Basically the same code
    as in GHC.
-}
module AgdaLight.Syntax.Parser.StringLiterals
    ( litString, litChar
    ) where
...
import AgdaLight.Syntax.Parser.LookAhead
import AgdaLight.Syntax.Position
import AgdaLight.Syntax.Literal

import AgdaLight.Utils.Char   ( decDigit, hexDigit, octDigit )
...
stringToken del mkTok inp inp' n =
    do  setLastPos (backupPos $ lexPos inp')
        ...
        -- AgdaLight.Syntax.Parser.LexActions.token.
