...
-}
module AgdaLight.Syntax.Parser.Layout
...
{-| Executed for the first token in each line (see 'AgdaLight.Syntax.Parser.Lexer.bol').
    ...
    - /to the left/ :
        ...
        'AgdaLight.Syntax.Parser.Lexer.bol' state).

    - /same column/ :
        Exit the 'AgdaLight.Syntax.Parser.Lexer.bol' state and return a virtual semi
        colon.

    - /to the right/ :
        Exit the 'AgdaLight.Syntax.Parser.Lexer.bol' state and continue lexing.
...
{-| This action is only executed from the 'AgdaLight.Syntax.Parser.Lexer.empty_layout'
    state. It will exit this state, enter the 'AgdaLight.Syntax.Parser.Lexer.bol' state,
...
{-| Start a new layout context. This is one of two ways to get out of the
    'AgdaLight.Syntax.Parser.Lexer.layout' state (the other is 'openBrace'). There are
    two possibilities:
    ...
    block so we enter the 'AgdaLight.Syntax.Parser.Lexer.empty_layout' state. In both
